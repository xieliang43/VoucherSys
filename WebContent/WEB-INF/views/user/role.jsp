...
role = {
	all : ctx + '/sysRoleAction!loadAll.action',// 加载所有
	save : ctx + "/sysRoleAction!save.action",//保存
	del : ctx + "/sysRoleAction!delete.action",//删除
	allModules : ctx + "/sysModuleAction!loadTreeMenu.action",//系统菜单
	saveModules : ctx + "/sysModuleAction!saveRoleModules.action",//保存角色与系统菜单
	myModules : ctx + "/sysModuleAction!getSysModulesByRole.action",
	childNodes : '',
	pageSize : 20
	// 每页显示的记录数
};
			...
			listeners : {
				'cellclick' : function(obj, rowIndex, columnIndex, e) {
					...
					if (record) {
						...
						// 当前选择用户的资源信息
						Share.AjaxRequest({
									url : role.myModules + "?roleId=" + record.data.id,
...
role.delFun = function() {
	...
	Ext.Msg.confirm('提示', '确定要删除这条记录吗?', function(btn, text) {
				if (btn == 'yes') {
					// 发送请求
					Share.AjaxRequest({
								url : role.del + "?id=" + record.data.id,
								callback : function(json) {
									role.alwaysFun();
									role.store.reload();
								}
