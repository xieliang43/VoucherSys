...
<script type="text/javascript">
	...
	user = {
		all : ctx + '/sysMerchantAction!loadAll.action', // 所有用户
		save : ctx + "/sysMerchantAction!save.action",// 保存用户
		reset : ctx + "/sysMerchantAction!resetPassword.action",// 重置用户密码
		del : ctx + "/sysMerchantAction!delete.action",// 删除用户
		myRole : ctx + "/sysMerchantAction!loadUserRole.action",// 加载某个用户的所有角色
		loadRole : ctx + '/sysRoleAction!loadAll.action',// 加载所有角色
	...
	user.editAction = new Ext.Action({
		...
		handler : function() {
			...
			// 查找当前用户的角色
			Share.AjaxRequest({
				url : user.myRole + "?id=" + record.data.id,
	...
	user.resetPwdFun = function() {
		...
		Ext.Msg.confirm('提示', '确定要重置此用户的密码吗?', function(btn, text) {
			if (btn == 'yes') {
				// 发送请求
				Share.AjaxRequest({
					url : user.reset + "?id=" + record.data.id,
					callback : function(json) {
						user.alwaysFun();
						user.store.reload();
					}
	...
	user.delFun = function() {
		...
		Ext.Msg.confirm('提示', '确定要删除这条记录吗?', function(btn, text) {
			if (btn == 'yes') {
				// 发送请求
				Share.AjaxRequest({
					url : user.del + "?id=" + record.data.id,
					callback : function(json) {
						user.alwaysFun();
						user.store.reload();
					}
