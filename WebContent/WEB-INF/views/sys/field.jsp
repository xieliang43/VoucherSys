...
field = {
	all : ctx + '/sysFieldAction!loadAll.action',// 加载所有
	save : ctx + "/sysFieldAction!save.action",//保存
	del : ctx + "/sysFieldAction!delete.action",//删除
	synchro : ctx + "/sysFieldAction!synchro.action",//内存同步
...
field.delFun = function() {
	...
	Ext.Msg.confirm('提示', '确定要删除这条记录吗?', function(btn, text) {
				if (btn == 'yes') {
					// 发送请求
					Share.AjaxRequest({
								url : field.del + "?id=" + record.data.id,
								callback : function(json) {
									field.alwaysFun();
									field.store.reload();
								}
