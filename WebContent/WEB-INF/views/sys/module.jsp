...
module = {
	all : ctx + '/sysModuleAction!loadAll.action',// 加载所有
	save : ctx + "/sysModuleAction!save.action",//保存
	del : ctx + "/sysModuleAction!delete.action",//删除
...
module.delFun = function() {
	...
	Ext.Msg.confirm('提示', '你真的要删除选中菜单及其包含的所有子菜单吗?', function(btn, text) {
				if (btn == 'yes') {
					// 发送请求
					Share.AjaxRequest({
								url : module.del + "?id=" + record.data.id,
								callback : function(json) {
									module.alwaysFun();
									module.store.reload();
								}
